// Same streaming shape as ../promote/route.ts, but read-only —
// runVerifyAction never writes to either repository. Works in either
// direction (unlike promote/confirm/reconcile), so no direction
// validation happens here or in the action itself.
import type { NextRequest } from "next/server";
import { loadConfig } from "prismic-migration/config";
import { runVerifyAction } from "prismic-migration/actions";
...
export async function GET(req: NextRequest) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      const runLog = await createRunLog("verify", from, to);
      ...
      try {
        ...
        const result = await runWithLogSink(sink, () => runVerifyAction(config, from, to));
        send("done", result);
      } catch (err) {
