  ...
  return {
    log(level: LogLevel, message: string, attributes?: LogAttributes, err?: unknown) {
      ...
      if (process.env.NODE_ENV === "production") {
        // Production: JSON Lines format (one JSON object per line) on
        // stdout — this is the format ECS/CloudWatch forwards, and what a
        // Splunk log-ingestion pipeline (HEC forwarder / Splunk Add-on for
        // AWS / Cribl, whichever this org uses) expects to parse as
        // structured fields rather than a single opaque message string.
